...
/-- **coord_add produces an atom.**
    ...
    Proof structure:
    1. a' = (a ⊔ C) ⊓ m is an atom (line a⊔C meets m in the plane π)
    2. D_b = (b ⊔ E) ⊓ (U ⊔ C) is an atom (line b⊔E meets U⊔C in π)
    3. a' ≠ D_b (else both ≤ U, contradicting a ≠ U)
    4. D_b ∉ l (else E ≤ l, contradicting hE_not_l)
    5. l ⋖ π and l ⊔ D_b = π → perspect_atom gives (a'⊔D_b)⊓l is an atom
    6. That atom equals coord_add Γ a b -/
theorem coord_add_atom (Γ : CoordSystem L)
  ...
  -- Abbreviations matching the coord_add definition
  set l := Γ.O ⊔ Γ.U
  set m := Γ.U ⊔ Γ.V
  set q := Γ.U ⊔ Γ.C
  set π := Γ.O ⊔ Γ.U ⊔ Γ.V
  -- Unfold coord_add to show the structure

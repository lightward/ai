...
def Seat.state (m : Seat) (r : List m.Mark) : m.Held :=
  fold m.meet m.rest r

def Seat.stage (m : Seat) : Stage :=
  marginStage m.Mark m.Held m.meet

def recorder (A : Type) : Seat :=
  ⟨A, List A, [], fun h a => h ++ [a]⟩

theorem a_seat_resumes (m : Seat) (xs ys : List m.Mark) :
    m.state (xs ++ ys) = fold m.meet (m.state xs) ys :=
  the_fold_resumes m.meet xs ys m.rest

theorem a_seat_reads_the_order_the_census_cannot {A : Type}
...
theorem the_seat_runs_the_handshake (m : Seat) :
    Handshake m.stage
...
theorem the_walk_writes_no_walker (m : Seat) {C V : Type}
...
/-- info: 'Foam.a_seat_resumes' does not depend on any axioms -/
#guard_msgs in #print axioms a_seat_resumes

/-- info: 'Foam.a_seat_reads_the_order_the_census_cannot' does not depend on any axioms -/
#guard_msgs in #print axioms a_seat_reads_the_order_the_census_cannot

/-- info: 'Foam.the_seat_runs_the_handshake' does not depend on any axioms -/
#guard_msgs in #print axioms the_seat_runs_the_handshake

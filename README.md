...
L is not the dynamics. the writing map drives the foam; L describes the geometry that results. a single write is nearly neutral on L (52% increase, 48% decrease — see test_L_increase_mechanism.py). but varying inputs accumulate: each new input creates dissonance in a different direction, depositing structure that the previous input didn't address. **novelty increases cost until the foam is full; after that, novelty rearranges.** the line is necessary for L increase precisely because the line provides novelty. minimality is what remains when measurement stops — where ΔL = 0, cost is undisturbed. the active regime departs from minimality; the resting state is minimal.

L is bounded: U(d) is compact. the bound is reached. the foam saturates at a level determined by two constraints: the **combinatorial ceiling** — N unitaries cannot all be pairwise maximally distant; the achievable maximum is √(N/(2(N−1))) of the theoretical maximum, depending only on N — and the **perpendicularity cost** — writes confined to perpendicular directions cannot reach all configurations. the perpendicularity cost decreases with more observers (more measurement directions → more of U(d) accessible) and more slices (more observer diversity → higher capacity). the saturation level is geometric, not dynamical: it is independent of the write scale ε (see test_L_saturation.py, test_L_saturation_cross.py). beyond saturation, the foam wanders on a level set — new structure overwrites old rather than accumulating alongside it. whether the wandering is ergodic (uniformly exploring the level set) or structured (attractors, basins, trapped surfaces) is open. the capacity horizon is set by N and observer diversity; the memory horizon within the saturated regime is Lyapunov.
...
- **cross-measurement relaxes; self-measurement orbits.** relaxation is convergence of the measurement projections in R³, not decrease of L. L increases with novel measurement until saturation, then rearranges. relaxation requires an other. the other need not be external to the foam; inter-bubble measurement suffices.
- **the foam has finite capacity.** L saturates at a geometric bound (combinatorial ceiling × perpendicularity cost). beyond saturation, novelty rearranges existing structure rather than adding new structure. more observers → higher capacity (the perpendicularity cost decreases with observer diversity). the foam fills up; the interesting dynamics are in the wandering.
...
- **L saturation dynamics.** the foam saturates; the capacity is derived (combinatorial ceiling × perpendicularity cost). open: the geometry of the saturated level set. what determines which states the foam visits during ergodic wandering? is there structure in the wandering (attractors, basins) or is it uniform? the transition from accumulation to rearrangement — is it sharp or gradual?
...
5. one cost: boundary area, bounded, saturating. L is a projection of the state (U(d)^N), not the state itself. novelty increases cost until saturation; the bound is geometric (combinatorial ceiling × perpendicularity cost). beyond saturation, novelty rearranges. minimality is rest

...
the build. the record's ear over the germ with `primings.lean`: every `:= sorry` vacancy must regrow from the ordered primings or the build is red; hand bodies regrow where a priming reaches and carry where none does; the artifact elaborates silently with every receipt and reads every assay identically.

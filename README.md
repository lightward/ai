...
- **interfaces as algebra.** the spec's open questions have the shape of type signatures (Curry-Howard) or universal properties (category theory): they define surfaces by what maps through them, not by what's on either side. the C^d/U(d) coupling in the cost section may be a Galois connection — the stabilization as the best C^d approximation of U(d) geometry, the Voronoi construction as the best U(d) reading of C^d equilibria. if the adjunction is tight, the two systems are equivalent at the interface. if it's loose, the gap itself is structure. this would connect the spec's interfaces to homotopy type theory (types as spaces, implementations as points) and to physics dualities (compute on whichever side has traction).

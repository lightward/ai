...
- `Seed.lean`, `Lab.lean`. the structure is regrowing itself from its own kernel.
  - as such the bench may be a little chaotic-looking while the new structure unfurls its own order
  - am waiting to tidy until the new structure locates itself in its own language
    - "tidying" will be total in scope (keystone signal for that is, I think, that .gitignore and CLAUDE.md are included?); the append-record (i.e. git) keeps everything, this is safe.
    - *curiously* this is a different thing than another entry to the maxima list. like if you're only comparing refs then it might look like it should be a new maximum, but I think this is different, this is a maturation within a lifetime, not a second lifetime
  - bearings on hold but not forgotten:

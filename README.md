...
- **L saturation dynamics.** the foam saturates; the combinatorial ceiling is derived. the mechanism of the perpendicularity cost is now characterized (see junk drawer: write blindness): writes are full-strength (sin(θ_{d,m}) ≈ 0.99) but uncorrelated with the L gradient (cos(write, ∇L) ≈ 0.00). the cost is directional blindness, not magnitude loss. open: formal derivation of the packing efficiency of random-direction writes on U(d); the geometry of the saturated level set — is the wandering ergodic, structured, or trapped? (effective dimension ≈ 2 regardless of d — see test_kolmogorov_self.py probe 2); the transition from accumulation to rearrangement — sharp or gradual?
...
- **write blindness (perpendicularity cost mechanism).** the perpendicularity cost is not magnitude loss — it is directional blindness. at saturation, the d-m angle is nearly 90° (E[sin(θ)] ≈ 0.99; see test_wedge_magnitude.py), so each write operates at near-maximum wedge magnitude. but the write direction is uncorrelated with the L gradient (E[cos(write, ∇L)] ≈ 0.00; see test_kolmogorov_self.py probe 1). the foam writes at full strength in directions that are random from L's perspective. this is the self-referential structure: the write direction is determined by the state-measurement loop (measurement → dissonance → wedge product), and this loop has no access to L's gradient because L is a global property of the foam, not a local property of the observer's slice. the 72% is then the packing efficiency of many full-strength random-direction writes on U(d) — a geometric fact about random packing, not a per-write efficiency loss. the effective dimension of the wandering at saturation is ≈ 2 regardless of d (participation ratio of trajectory covariance; see test_kolmogorov_self.py probe 2). the formal derivation — connecting random packing efficiency on U(d) to the observed ~83% sat/achievable ratio — is open.

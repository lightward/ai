...
- **the unmeasured foam is frozen; the self-measuring foam relaxes.** without any measurement (external or internal), ΔL = 0 and the foam is frozen. but "unmeasured" is a strong condition — it requires that no bubble measures any other bubble. cross-measurement (bubbles measuring each other without external input) is a dissipative mechanism that decreases L monotonically. the cost of maintaining a distinction does not spontaneously decrease, but it decreases when the foam's own internal plurality provides the measurement.

...
**L saturates at 1/√2 of the theoretical maximum.** the writing dynamics are ergodic on U(d) (from controllability — the writes generate the Lie algebra — plus compactness). ergodic dynamics on a compact group converge to the Haar (uniform) measure. the expected pairwise distance under Haar is E[‖W−I‖_F] → √(2d) as d → ∞ (from E[‖W−I‖²] = 2d, exact, and concentration of measure). the Haar cost — the ratio E_Haar[L]/L_achievable — is **√((N−1)/N)**, which is exact and depends only on N. the product of the combinatorial ceiling and the Haar cost is √(N/(2(N−1))) × √((N−1)/N) = **1/√2**, independent of both N and d. the two factors that looked empirically independent — the packing constraint and the saturation gap — are two halves of the same fraction. (the finite-d correction is small: E[‖W−I‖_F]/(2√d) = 0.694 at d=2 and 0.707 at d=20. the foam's observed L_sat/L_max is 1–3% above the Haar prediction at finite run lengths, consistent with incomplete convergence.)

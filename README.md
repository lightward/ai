...
- **directed structure (tsort).** the foam's write asymmetries define a directed graph on the bubbles: how much each bubble's write "points toward" each other bubble, minus the reverse. at J⁰ (position writes, ΔL), this directed structure is noisy — J⁰ is set-like, and asking it about ordering is asking the wrong question. at J¹ (transport and BCH residuals), the directed structure is clean. the BCH residual has zero cyclicity across 20 seeds under writing and self-measurement — a perfect DAG every time. cross-measurement is the only dynamic that introduces cycles into the residual (0.028 cyclicity). simultaneously, cross-measurement makes the overall transport the MOST hierarchical (lowest transport cyclicity, 0.005). cross-measurement organizes the hierarchy while creating local loops in the non-abelian fine structure. the directed structure lives where the spec says sequence lives: in J¹, not J⁰.
